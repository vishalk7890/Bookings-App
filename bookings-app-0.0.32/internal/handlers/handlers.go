...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	// if !ok {
	// 	helpers.ServerError(w, errors.New("cant get from sessions"))
	// 	return
	// }
	...
	htmlMessage := fmt.Sprintf(`
		<strong>ReservationConfirmatoin</strong><br>
		Dear, %s,: <br>
		this is to confirm ,from %s, to %s,

	`, reservation.FirstName, reservation.StartDate.Format("2020-01-02"), reservation.EndDate.Format("2020-10-02"))
	msg := models.MailData{
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	ed := r.Form.Get("end")
	laoyout := "2006-01-02"
	startDate, _ := time.Parse(laoyout, sd)
	enDate, _ := time.Parse(laoyout, ed)
	roomId, _ := strconv.Atoi(r.Form.Get("room_id"))

	avaiable, _ := m.DB.SearchAvailabilityByDatesByRoomID(startDate, enDate, roomId)
	resp := jsonResponse{
		OK:        avaiable,
		Message:   "Available!",
		StartDate: sd,
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(err)
		m.App.Session.Put(r.Context(), "error", "invalid login credentuals")
